...
export const OrcamentoFormSchema = z.object({
    nome: z.string().nonempty("O campo não pode estar vazio").regex(/^[\p{L} ]+$/u, "Apenas letras são permitidas"),
    telefone: z.string().nonempty("O campo não pode estar vazio").regex(/^\(?\d{2}\)?\s?\d{4,5}-?\d{4}$/, "Apenas números são permitidos"),
